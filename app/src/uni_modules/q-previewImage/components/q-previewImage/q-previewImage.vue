					...
					<movable-view class="movable-view" direction="all" :inertia="true" damping="100" scale="true" scale-min="1" scale-max="4" :scale-value="scale">
						<scroll-view scroll-y="true" class="uni-scroll-view">
							<view class="scroll-view">
								<image class="image" :src="imagebaseurl + item" mode="widthFix" @longpress="onLongpress(item)" v-if="!item.includes('video')" />
								<video :src="imagebaseurl + item" class="slide-video" loop autoplay
								mode="aspectFill" :lazy-load="true" v-if="item.includes('video')" controls 
								:enable-progress-gesture="false" :poster="imagebaseurl + item + '?x-oss-process=video/snapshot,t_0,f_jpg'" />
							</view>
						</scroll-view>
					</movable-view>
				</movable-area>
